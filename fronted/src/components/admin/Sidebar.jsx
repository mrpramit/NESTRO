    ...
    for (const group of menuGroups) {
      for (const item of group.items) {
        if (item.hasSubmenu && item.submenuItems) {
          const hasActiveSub = item.submenuItems.some(
    ...
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen flex-col bg-[#1c2434] border-r border-[#e2e8f0] dark:border-[#2e3a47] transition-all duration-300 ease-in-out lg:static lg:translate-x-0 ${
          ...
          sidebarCollapsed
            ? "w-20 lg:w-20 overflow-visible"
            : "w-72.5 lg:w-72.5 overflow-y-hidden"
        }`}
      >
        {/* Sidebar Header */}
        <div
          className={`flex items-center justify-between gap-2 py-5.5 lg:py-6.5 ${sidebarCollapsed ? "px-4 justify-center" : "px-6"}`}
        >
        ...
        {/* Sidebar Menu Links */}
        <div
          className={`no-scrollbar flex flex-col duration-300 ease-linear flex-1 py-4 ${sidebarCollapsed ? "px-2 overflow-visible" : "px-4 lg:px-6 overflow-y-auto"}`}
        >
          <nav
            className={`mt-5 py-4 space-y-6 ${sidebarCollapsed ? "px-1 overflow-visible" : "px-2 lg:px-4"}`}
          >
                    ...
                    return (
                      <li key={itemIdx} className="relative group/item">
                        ...
                        >
                          ...
                          {!sidebarCollapsed && (
                            <>
                              ...
                              {item.hasSubmenu && (
                                <ChevronDownIcon
                                  className={`w-4 h-4 text-[#8a99ad] group-hover:text-white transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
                                />
                              )}
                            </>
                          )}
                        ...
                        {!sidebarCollapsed &&
                          ...
                          item.submenuItems.length > 0 && (
                            <div
                              className={`translate transform overflow-hidden transition-all duration-300 ${isExpanded ? "max-h-40 opacity-100 mt-2" : "max-h-0 opacity-0 pointer-events-none"}`}
                            >
                                  ...
                                  return (
                                    <li key={subIdx}>
                                      ...
                                      >
                                        {SubIcon && (
                                          <SubIcon
                                            className={`w-4 h-4 transition-colors duration-300 flex-shrink-0 ${isSubItemActive ? "text-white" : "text-[#8a99ad] group-hover:text-white"}`}
                                          />
                                        )}
                        ...
                        {sidebarCollapsed && (
                          <div
                            className={`absolute left-full top-0 ml-2 invisible opacity-0 translate-x-3 group-hover/item:visible group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300 ease-in-out z-[99] bg-[#1c2434] border border-slate-700/50 rounded-sm shadow-xl overflow-hidden ${
                              item.submenuItems && item.submenuItems.length > 0
                                ? "w-48"
                                : "w-max py-2 px-3 text-sm text-white font-medium"
                            }`}
                          >
                            {item.submenuItems &&
                            item.submenuItems.length > 0 ? (
                                      ...
                                      <li key={subIdx}>
                                        <Link
                                          ...
                                          className={`flex items-center gap-2 px-4 py-2 text-xs font-medium text-[#8a99ad] hover:text-white hover:bg-[#333a48] transition-colors duration-200 ${
                                            isSubItemActive
                                              ? "text-white bg-[#333a48]/50"
                                              : ""
                                          }`}
                                        >
                                          {SubIcon && (
                                            <SubIcon
                                              className={`w-3.5 h-3.5 transition-colors duration-200 ${isSubItemActive ? "text-white" : "text-[#8a99ad] group-hover:text-white"}`}
                                            />
                                          )}
                                          <span>{subItem.name}</span>
                                        </Link>
